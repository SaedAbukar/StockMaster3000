...
public class DashboardTab extends VerticalLayout implements LanguageChangeListener {
  ...
  private void createFilterButtons() {
    ...
    expiringButton = createFilterButton(getTranslation("inventory.filter_exp"), "EXPIRING",
        false);
    ...
    filterLayout.setJustifyContentMode(FlexComponent.JustifyContentMode.BETWEEN); // Filter
    // buttons left, Add button right
  ...
  private void createGrid() {
    ...
    grid.getElement().executeJs(
        "this.shadowRoot.querySelectorAll('tr').forEach(row => { "
            ...
            + "    row.addEventListener('mouseover', () => row.style.backgroundColor "
            + "= 'rgba(0, 150, 136, 0.1)'); "
  ...
  private void showAddProductDialog() {
    ...
    saveButton = new Button(getTranslation("save"), e -> {
      try {
        ...
        if (expirationDate.getValue() != null) {
          long daysUntilExpiration = ChronoUnit.DAYS.between(LocalDate.now(),
    ...
    VerticalLayout formLayout = new VerticalLayout(nameField, quantityField, priceField,
        expirationDate, supplierField, categoryWrapper);
  ...
  private void searchByName() {
    ...
    searchButton.addClickListener(event -> {
      ...
      if (!searchText.isEmpty()) {
        List<Product> products = productService.getProductsByName(inventory.getId(),
  ...
  public void updateTexts() {
    ...
    grid.getColumnByKey("days_until_expiration").setHeader(getTranslation(
